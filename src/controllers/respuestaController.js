...
const enviarRespuesta = async (req, res) => {
    ...
    try {
        ...
        // Paso 3: Guardar los hechos uno por uno para evitar exceder el límite de conexiones
        for (const respuesta of respuestas) {
            ...
            // Esperamos a que cada inserción termine antes de continuar con la siguiente
            await Hecho.createHecho(hechoData);
        }
...
const enviarRespuestaSinGuardar = async (req, res) => {
    ...
    try {
        ...
        // Llenar los hechos con las respuestas proporcionadas
        for (const respuesta of respuestas) {
            const { id_pregunta, respuesta_valor } = respuesta;
            const valorBooleano = (respuesta_valor === true || respuesta_valor === 'true' || respuesta_valor === 1);
            facts[String(id_pregunta)] = valorBooleano;
        ...
        res.json({ message: "Diagnóstico determinado exitosamente.", diagnosticoId, diagnostico });
    } catch (err) {
        console.error("Error al procesar las respuestas:", err);
        return res.status(500).json({ error: "Error interno del servidor." });
    }
};
...
module.exports = {
    enviarRespuesta,
    enviarRespuestaSinGuardar,
    enviarRespuestaProgreso
};